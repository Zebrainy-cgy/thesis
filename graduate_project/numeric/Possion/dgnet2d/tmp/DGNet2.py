...
class DGNet:
        ...
        for i in range(Nloc):
            ...
            for k in range(Nface):
                if meshface[k].bctype == 0:
                    CI, SI, JI2, JIG2 = inte(self.models, k, i)
                    lossterm1 += CI + SI
                    conloss += JI2 + JIG2
                elif meshface[k].bctype == 1:
                    CD, SD, DS, JD = inte(self.models, k, i)
                    lossterm1 += CD + SD - DS
                    bdloss += JD
            # tfloss[i] = (eqloss + bdloss) ** 2
            eqloss = lossterm1 ** 2
            # print(self.eqw * eqloss , self.conw * conloss/Nloc , self.bdw * bdloss/Nloc)
            tfloss[i] = self.eqw * eqloss + self.conw * conloss/Nloc + self.bdw * bdloss/Nloc
    ...
    def loadmodel(self):
        for i in range(Nelt):
            net = basisref()
            para = torch.load(f"./model/net{i}.pth1")
